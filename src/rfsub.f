...
c     SUBROUTINE BUILDTREE
     ...
     1     nodeclass, ndbigtree, win, wr, wl, mred, nuse, mind, fwt)
...
c     main program.
     ...
     1     wl(nclass), tgini(mdim), xrand, fwt(mdim), fwt1(mdim)
      integer msplit, ntie
...
c     initialize for next call to findbestsplit
     ...
     1        best,ncase, jstat,mtry,win,wr,wl,mred,mind,fwt1)
cc          print *, "findtree..."
...
c     add weight of feature fwt1
     ...
     3     mred, mind, fwt1)
      implicit double precision(a-h,o-z)
      integer a(mdim,nsample), cl(nsample), cat(mdim),
     1     ncase(nsample), b(mdim,nsample), nn, j, tmp, whichfeature
      double precision tclasspop(nclass), tclasscat(nclass,53), dn(53),
     1     win(nsample), wr(nclass), wl(nclass), xrand, fwt1(mdim)
...
c     sampling mtry variables w/o replacement.
      do mt = 1, mtry
         call rrand(xrand)
cc         j = int(nn * xrand) + 1
         j = whichfeature(xrand,fwt1,nn)
cc         print *, "nn :", nn
ccc	 if (mt .eq. 1) then
ccc            open (unit=7,file="test1.txt")
ccc            write (7,*) "j is ",j
ccc            close (7)
ccc         end if
cc         if (mt .eq. 1) then
         print *, 'split :', mt,' feature : ', mind(j)
cc         do i=1,nn
cc             print *, i, "thweight: ", fwt1(i)
cc         end do
cc         end if
         tmp = fwt1(j)
         fwt1(j) = fwt1(nn)
...
cc add a function here
      function whichfeature(xrand,fwt1,nfeatures)
cc	implicit none
cc        logical :: exist
        integer :: whichfeature
        integer i, nfeatures
        double precision segment(nfeatures+1), xrand, fwt1(nfeatures),tot,total
        segment(1)=0.0d0
        segment(nfeatures+1)=1.0d0 
	tot=0.0d0
        total=0.0d0
cc        print *, "xrand :",xrand
          if (nfeatures >1) then
cc             inquire(file="weight.txt", exist=exist)
cc             if (exist) then
cc                open(12, file="weight.txt", STATUS="OLD")
cc             else
cc                open(12, file="weight.txt", status="new")
cc             end if
cc             do i=1,nfeatures
cc               write (12,*)  i, "thweight: ", fwt1(i)
cc             end do
cc             close (12)
cc          do i=1,nfeatures
cc             print *, i, "thweight: ", fwt1(i)
cc          end do
	  do i=1,nfeatures
          ...
          do i=2,nfeatures
             tot = tot+fwt1(i-1)/total
             segment(i) = tot
          end do
cc          print *, "nfeatures: ", nfeatures
cc          print *, "total :", total
cc          print *,  "tot :", tot
cc          inquire(file="seg.txt", exist=exist)
cc             if (exist) then
cc                open(11, file="seg.txt",  STATUS="OLD")
cc             else
cc                open(11, file="seg.txt", status="new")
cc             end if
cc             do i=1,nfeatures+1
cc               write (11,*)   i, "seg: ", segment(i)
cc             end do
cc             close (11)
cc          do i=1,nfeatures+1
cc             print *, i, "seg: ", segment(i)
cc          end do
          do i=1,nfeatures
              if (xrand < segment(i+1) .AND. segment(i) <= xrand) then
                  whichfeature = i  
              end if
         end do
       else
